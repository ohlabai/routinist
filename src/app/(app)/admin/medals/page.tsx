...
// 어드민 — 메달 신청 관리 (build 113).
// 접수 → 결제 → 발송 → 배송완료 흐름. 송장번호 + 메모.
...
export default function AdminMedalsPage() {
  ...
  const [rows, setRows] = useState<MedalRequest[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('admin_list_medal_requests', {
        p_status: filter === 'all' ? null : filter,
        p_limit: 200,
      });
      if (error) throw error;
      setRows((data ?? []) as MedalRequest[]);
    } catch (e) {
  ...
  const key = (r: MedalRequest) => `${r.user_id}:${r.course_id}`;

  const toggle = (r: MedalRequest) => {
    const k = key(r);
    setOpened(prev => {
  ...
  const save = async (r: MedalRequest) => {
    const k = key(r);
    const d = drafts[k];
    if (!d) return;
    setSaving(k);
    try {
      const supabase = getSupabase();
      const { error } = await supabase.rpc('admin_update_medal', {
        p_user_id: r.user_id,
  ...
  const copyAddress = async (r: MedalRequest) => {
      ...
      <header className="sticky top-0 z-30 bg-[var(--background)]/85 backdrop-blur-lg border-b border-[var(--card-border)]/30">
        <div className="flex items-center gap-2 px-3 py-3">
          ...
          <h1 className="text-xl font-extrabold tracking-tight inline-flex items-center gap-1.5">
            <Award size={18} className="text-amber-500" /> 메달 신청
          </h1>
              ...
              <article key={k} className="card p-4">
                ...
                {isOpen && (
                  <div className="mt-3 pt-3 border-t border-[var(--card-border)]/40 space-y-3">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[var(--muted)] mb-1">메모 (내부용)</label>
                      <input
                        value={d.note}
                        onChange={(e) => setDrafts(prev => ({ ...prev, [k]: { ...d, note: e.target.value } }))}
                        placeholder="결제 안내 보냄 / 재고 대기 등"
                        className="w-full px-3 py-2.5 rounded-xl border-2 border-[var(--card-border)] bg-[var(--background)] text-sm focus:outline-none focus:border-amber-500"
                      />
                    </div>
