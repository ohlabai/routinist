  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (weight !== null && (isNaN(weight) || weight < 20 || weight > 250)) {
        setToast({ text: '체중은 20~250kg 사이로 입력해주세요', tone: 'warn' });
        setSavingSettings(false);
        return;
      }
      if (maxHr !== null && (isNaN(maxHr) || maxHr < 100 || maxHr > 230)) {
        setToast({ text: '최대 심박수는 100~230 사이로 입력해주세요', tone: 'warn' });
        setSavingSettings(false);
        return;
      }
  ...
  return (
    <div className="max-w-lg mx-auto pb-24 bg-[var(--background)] min-h-screen">
      ...
      <header className="sticky top-0 z-30 bg-[var(--background)]/80 backdrop-blur-lg border-b border-[var(--card-border)]/30 flex items-center gap-2 px-3 py-3">
        ...
        <h1 className="text-xl font-extrabold tracking-tight flex-1">러닝 코치 <span className="text-xs text-violet-500 font-bold">AI</span></h1>
        <button onClick={() => setShowSettings(s => !s)} aria-label="설정"
          className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-violet-50 dark:hover:bg-violet-950/30 active:scale-90">
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        {today ? (
          <div className={`card p-6 bg-gradient-to-br ${scoreColor} text-white shadow-lg`}>
            ...
            <div className="mt-4 pt-4 border-t border-white/20 grid grid-cols-3 gap-2 text-center">
              <div>
                <p className="text-[10px] font-bold text-white/70 uppercase">장기 피트니스</p>
                <p className="text-lg font-extrabold tabular-nums">{today.ctl}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-white/70 uppercase">최근 부하</p>
                <p className="text-lg font-extrabold tabular-nums">{today.atl}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-white/70 uppercase">컨디션</p>
        ...
        {trend.length > 0 && (
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-extrabold">최근 14일 부하 흐름</h3>
            </div>
            <MiniChart data={trend.slice(-14)} />
            <p className="text-[11px] text-[var(--muted)] mt-3 leading-relaxed">
              <span className="inline-block w-2 h-2 rounded-full bg-violet-400 mr-1" /> 장기 피트니스 (꾸준함)
              {'  '}<span className="inline-block w-2 h-2 rounded-full bg-amber-400 mr-1" /> 최근 부하 (피로)
            </p>
          </div>
        )}
        ...
        <div className="card p-4 bg-violet-50/40 dark:bg-violet-950/15 border-violet-200/40 dark:border-violet-900/30 flex items-start gap-2.5">
          ...
          <div>
            <p className="text-xs font-extrabold text-violet-700 dark:text-violet-300">코칭은 어떻게 계산되나요?</p>
            <p className="text-[11px] text-[var(--muted)] mt-1 leading-relaxed">
              거리 · 시간 기반으로 매일 부하 점수를 매기고, 장기 평균(42일)과 단기 평균(7일)의 차이로 오늘 컨디션을 산출해요.
              체중·최대 심박수 입력 시 더 정확해져요.
            </p>
          </div>
        </div>
        ...
        {showSettings && (
          <div className="card p-5 space-y-3 border-violet-200 dark:border-violet-900/40">
            ...
            <div>
              <label className="text-[11px] font-bold text-[var(--muted)] inline-flex items-center gap-1 mb-1">
                <Heart size={11} /> 최대 심박수 — HR Zones 분석용 (220 - 나이 가능)
              </label>
              <input
