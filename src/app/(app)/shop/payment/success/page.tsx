  ...
  useEffect(() => {
    ...
    fetch('/api/payments/toss/confirm', {
      ...
      .then(async r => {
        ...
        if (!r.ok || !json.success) throw new Error(json.message || json.error || tt('결제 확정 실패'));
        setStatus('success');
      ...
      .catch(e => {
        ...
        setStatus('fail');
        setErrorMsg(e instanceof Error ? e.message : tt('결제 확정 실패'));
      });
  }, [searchParams, tt]);
  ...
  if (status === 'confirming') {
    return (
      <div className="max-w-lg mx-auto px-6 py-32 text-center bg-[var(--background)] min-h-screen">
        ...
        <p className="text-base font-extrabold text-[var(--foreground)] mb-1">{tt('결제 확인 중...')}</p>
        <p className="text-sm text-[var(--muted)]">{locale === 'en' ? 'One moment please' : '잠시만 기다려주세요'}</p>
      </div>
    );
  }
  ...
  if (status === 'fail') {
    return (
      <div className="max-w-lg mx-auto px-6 py-20 text-center bg-[var(--background)] min-h-screen">
        ...
        <h1 className="text-xl font-extrabold mb-2">{tt('결제 확정 실패')}</h1>
        ...
        <p className="text-xs text-[var(--muted)] mb-7 max-w-xs mx-auto break-keep">
          {locale === 'en' ? (
            <>Payment succeeded but order processing failed.<br />Contact support for fast assistance.</>
          ) : (
            <>결제는 완료됐는데 주문 처리 중 문제가 생겼어요.<br />고객센터에 문의하시면 빠르게 도와드릴게요.</>
          )}
        </p>
    ...
    <div className="max-w-lg mx-auto px-6 py-16 bg-[var(--background)] min-h-screen">
      ...
      <div className="text-center pt-8">
        ...
        <h1 className="text-3xl font-extrabold text-[var(--foreground)] mb-2 leading-tight">
          {tt('결제 완료!')}<br />
          <span className="text-emerald-600">{tt('주문이 접수됐어요')}</span>
        </h1>
        <p className="text-sm text-[var(--muted)] mt-2">
          {locale === 'en' ? "We'll ship your order soon" : '빠른 시일 내에 발송 처리해 드릴게요'}
        </p>
      </div>
      ...
      <div className="mt-12 space-y-2.5">
        ...
        >
          <ShoppingBag size={18} />
          {locale === 'en' ? 'View order details' : '주문 상세 보기'}
