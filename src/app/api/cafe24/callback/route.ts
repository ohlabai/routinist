...
/** 2026-08-06 보안 (리뷰 P1): 반사형 XSS 차단 — 외부 입력을 HTML 로 되돌려주지 않는다.
 *  앱과 같은 오리진이라 스크립트가 돌면 localStorage 의 Supabase 세션이 털린다. */
...
export async function GET(req: NextRequest) {
  ...
  if (errorParam) {
    // 값 자체를 렌더하지 않는다 (공격자가 URL 을 유포할 수 있는 유일한 무인증 진입점).
    return NextResponse.json({ error: 'oauth_denied' }, { status: 400 });
  }
  if (!code) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return new NextResponse(`<h1>토큰 교환 실패</h1><pre>${escapeHtml(text.slice(0, 500))}</pre>`, {
        status: 502, headers: { 'Content-Type': 'text/html; charset=utf-8' }
      });
    }
    token = await r.json() as TokenResponse;
  } catch (e) {
    return new NextResponse(`<h1>토큰 교환 예외</h1><pre>${escapeHtml((e as Error).message)}</pre>`, {
      status: 502, headers: { 'Content-Type': 'text/html; charset=utf-8' }
    });
  }
  ...
  if (error) {
    return new NextResponse(`<h1>DB 저장 실패</h1><pre>${escapeHtml(error.message)}</pre>`, {
...
<p>shop_no: ${escapeHtml(String(token.shop_no ?? ''))} · scopes: ${escapeHtml((token.scopes ?? []).join(', '))}</p>
<p>이 창은 닫으셔도 됩니다.</p>
