...
// `excludeId` 를 주면 직전 명언과 다른 명언이 나옴.
//
// server-side ORDER BY random() — 클라이언트 limit(50) 패턴은 PostgREST 가 같은 50개만 반환
// 해서 다양성 회귀가 있었음. RPC 로 1095개 풀 균일 확률 보장.
export async function fetchRandomQuote(
  ...
  try {
    const supabase = getSupabase();
    const { data, error } = await withTimeout(
      supabase.rpc('random_quote', { p_lang: lang, p_exclude_id: excludeId ?? null }),
      5000,
      'random_quote rpc',
    );
    if (error) throw error;
    ...
    console.warn('random_quote RPC 실패, fallback:', err);
