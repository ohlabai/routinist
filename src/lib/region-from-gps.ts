...
const WORLD: CoarseRegion[] = [
  ...
  // build 220 #5: 우리 룰 = 2단계 (국가 + 시·도/주/성).
  // 사용자 사례: 중국 저장성 이우 (29.30, 120.07) → '중국 저장' 표시되어야 함.
  // 항저우/광저우 (좁은 도시 bbox) 가 province bbox 보다 먼저 매칭되면 도시명만 표시되니
  // 도시 단위는 직할시 (베이징/상하이) 만 유지, 나머지는 성(省) 단위로 교체.
  ...
  { name: '중국 저장', bbox: [27.00, 31.30, 118.00, 123.00] },   // 항저우/이우/닝보 등
  { name: '중국 광둥', bbox: [20.20, 25.50, 109.70, 117.20] },   // 광저우/선전 등
  { name: '중국 장쑤', bbox: [30.70, 35.20, 116.30, 121.95] },   // 난징/쑤저우 등
  { name: '중국 산둥', bbox: [34.30, 38.40, 114.80, 122.70] },   // 칭다오/지난
  { name: '중국 푸젠', bbox: [23.50, 28.30, 115.80, 120.50] },   // 샤먼/푸저우
  { name: '중국 쓰촨', bbox: [26.00, 34.30, 97.30, 108.60] },     // 청두
  { name: '중국 후베이', bbox: [29.00, 33.30, 108.30, 116.10] },   // 우한
  { name: '중국 산시(陝)', bbox: [31.40, 39.60, 105.50, 111.20] }, // 시안
  { name: '중국 랴오닝', bbox: [38.70, 43.30, 118.80, 125.80] },   // 다롄/선양
  { name: '중국 헤이룽장', bbox: [43.40, 53.60, 121.20, 135.10] }, // 하얼빈
  { name: '중국 윈난', bbox: [21.10, 29.20, 97.50, 106.20] },     // 쿤밍
  { name: '중국 허난', bbox: [31.40, 36.40, 110.30, 116.70] },     // 정저우
  { name: '중국 후난', bbox: [24.60, 30.10, 108.80, 114.30] },     // 창사
  { name: '중국 안후이', bbox: [29.40, 34.70, 114.80, 119.40] },   // 허페이
  { name: '중국 허베이', bbox: [36.00, 42.70, 113.40, 119.90] },   // 스자좡 (베이징/톈진 둘러쌈)
  { name: '중국 광시', bbox: [20.80, 26.40, 104.30, 112.00] },     // 난닝
  { name: '중국 장시', bbox: [24.40, 30.10, 113.50, 118.50] },     // 난창
  { name: '중국 하이난', bbox: [18.10, 20.20, 108.50, 111.10] },   // 하이커우 (섬)
  ...
  // build 287: 포르투갈을 스페인보다 먼저. 이전엔 포르투갈 좌표 (lng -9.5 ~ -6.2) 가
  // 스페인 bbox (lng -10.0 ~ 4.5) 에 통째로 흡수돼 "스페인" 으로 잘못 표시됨 (hans 2026-06-13 포르토 사례).
