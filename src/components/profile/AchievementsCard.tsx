    ...
    <div className="card p-4">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <h3 className="text-sm font-extrabold">달성한 배지</h3>
        </div>
        ...
        {list.map(a => {
          ...
          return (
            <div key={a.code} className="rounded-xl bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-950/30 dark:to-yellow-950/20 border border-amber-200/60 dark:border-amber-800/40 p-2.5 text-center">
              ...
              <p className="text-[10px] font-extrabold mt-1 text-amber-900 dark:text-amber-200 leading-tight">{def.name}</p>
              <p className="text-[9px] text-amber-700/80 dark:text-amber-300/80 mt-0.5 line-clamp-1">{def.description}</p>
            </div>
          );
        })}
      </div>
